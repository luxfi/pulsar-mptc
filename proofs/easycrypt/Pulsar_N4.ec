...
(* -------------------------------------------------------------------- *)
(* STATUS: CLOSED. 0 admits across the file. The top-level lemma is     *)
(* discharged via `conseq (reshare_preserves_secret ...) => /#.` The     *)
(* Shamir-zero re-randomisation property is a section-local              *)
(* `declare axiom` over the abstract module R (a refinement obligation   *)
(* discharged when a concrete reshare implementation is plugged in).    *)
(* The auxiliary binding-invariant lemma is discharged via               *)
(* `proc true; auto.` (its postcondition is a `true` placeholder).      *)
(* -------------------------------------------------------------------- *)
(* Claim:                                                                *)
