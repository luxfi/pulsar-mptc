...
(*                                                                      *)
(* Discharge path (extraction DONE, refinement OPEN):                   *)
(*   1. `scripts/extract-jasmin-ec.sh` runs `jasmin2ec` over           *)
(*      `jasmin/threshold/combine.jazz`, producing                      *)
(*      `proofs/easycrypt/extraction/build/combine.ec` (3610 lines,    *)
(*      type-checks clean every CI push).                              *)
(*   2. The extracted module is `M` with procedure                      *)
(*        proc pulsar_combine(c_tilde_ptr, t0_ptr,                     *)
(*                            round2_msgs_ptr, threshold,              *)
(*                            sig_out_ptr) : W64.t                     *)
(*      at combine.ec:3245.                                             *)
(*   3. The refinement obligation:                                      *)
(*        equiv [ M.pulsar_combine ~ CombineAbs.combine :              *)
(*                  layout_invariant arg{1} arg{2}                     *)
(*                ==> read_signature_at(sig_out_ptr{1}) = res{2} ]      *)
(*      is OPEN and tracked in                                          *)
(*      https://github.com/luxfi/pulsar-mptc/issues/4.                  *)
(*                                                                      *)
(* Status: extraction DONE (CI gate enforces it). Byte-level            *)
(* refinement OPEN (#4). The section-local axiom stands as the         *)
(* hand-off contract pointing at concrete extracted artefacts +         *)
(* a tracked issue — NOT at an imaginary upstream theorem.              *)
...
(*   1. `scripts/extract-jasmin-ec.sh` runs `jasmin2ec` over the       *)
(*      libjade Dilithium sign source, producing                        *)
(*      `proofs/easycrypt/extraction/build/sign.ec` (4113 lines,       *)
(*      type-checks clean every CI push).                              *)
(*   2. The extracted module is `M(SC)` with                            *)
(*        proc sign(ptr_signature, ptr_m, m_len, ptr_sk) : ...         *)
(*      at sign.ec:3603.                                                *)
(*   3. The refinement obligation:                                      *)
(*        equiv [ M.sign ~ FIPS204Sign.sign :                          *)
(*                  layout_invariant arg{1} arg{2}                     *)
(*                ==> read_sig_at(ptr_signature{1}) = res{2} ]          *)
(*      which bridges through MLDSA65_Functional.fips204_sign           *)
(*      (lemmas/MLDSA65_Functional.ec) on the right side.              *)
(*                                                                      *)
(* Status: extraction works (CI gate enforces it). The byte-level      *)
(* refinement is open and tracked in                                   *)
(* https://github.com/luxfi/pulsar-mptc/issues/3. Until that's         *)
(* discharged, this section-local axiom stands as the hand-off contract.*)
