...
| Spec ↔ Go-reference protocol drift | **CLOSED — two-variant submission** | The submission supports **both** the v0.1 reveal-and-aggregate flow (Go reference, `D_i = cSHAKE(mask ‖ masked ‖ τ_1)`, KAT vectors and 19/19 interop subtests here) AND the v0.2 FSwA Lagrange-linearity flow (spec + Jasmin sources, `D_i = cSHAKE(pack_w1(w_i), τ_1)`). v0.1 ships as the runnable reference with the documented "v0.1 trust model caveat" (aggregator reconstructs the secret seed in memory); v0.2 is the high-assurance target that the Jasmin sources type-check against and the EasyCrypt N1 reduction is stated against. Both variants produce a FIPS 204 byte-equal `σ = (c̃, z, h)`; they differ in protocol shape, not output format. The spec (`pulsar-mptc/spec/pulsar.tex` §4.1 "Trust model variants") describes both with selection at submission-package generation time. |
