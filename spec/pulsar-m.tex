% Pulsar-M Technical Specification --- NIST MPTC submission
% Target deadline: 2026-Nov-16
%
% Compile: latexmk -pdf pulsar-m.tex
%
...
\begin{abstract}
Pulsar-M is a $t$-of-$n$ threshold implementation of NIST FIPS 204
...
produced by Pulsar-M verifies against the same group public key
...
Pulsar-M signatures with no code change. This collapses the
threshold/non-threshold split at the wire format: a relying party
who can verify ML-DSA can verify Pulsar-M.

\paragraph{Why Pulsar-M alongside Pulsar.}
The companion scheme Pulsar~\cite{ringtail2024} (R-LWE) is a
...
verifier). Pulsar-M ports that protocol's $2$-round signing structure
...
Pulsar-M consists of four protocols and one verifier:
...
$\Zq$. Pulsar-M operates over $\Rq$ with $(q, n) = (8\,380\,417, 256)$
...
string. The canonical customisation tags used by Pulsar-M are:
...
\midrule
DKG commit-digest (Round~1.5)            & \texttt{PULSAR-M-DKG-COMMIT-V1} \\
DKG transcript hash                       & \texttt{PULSAR-M-DKG-TRANSCRIPT-V1} \\
Sign Round-1 transcript (sid hash $H_1$)  & \texttt{PULSAR-M-SIGN-R1-V1} \\
Sign Round-2 challenge (FIPS~204 $\tilde c$) & FIPS~204 inner SHAKE-256 \\
Sign per-party PRNG (round-keyed)         & \texttt{PULSAR-M-SIGN-PRNG-V1} \\
Sign PRF for mask cancellation            & \texttt{PULSAR-M-SIGN-PRF-V1} \\
Reshare commit digest                      & \texttt{PULSAR-M-RESHARE-COMMIT-V1} \\
Reshare transcript                         & \texttt{PULSAR-M-RESHARE-TRANSCRIPT-V1} \\
Reshare beacon binding                     & \texttt{PULSAR-M-RESHARE-BEACON-V1} \\
Public matrix $\mathbf{A}$ derivation       & FIPS~204 \texttt{ExpandA} (no change) \\
Public matrix $\mathbf{B}$ derivation       & \texttt{PULSAR-M-EXPANDB-V1} \\
Complaint signing transcript               & \texttt{PULSAR-M-COMPLAINT-V1} \\
\bottomrule
\end{tabular}
\end{center}

The bound on customisation-string length is governed by
SP~800-185~\S3.2 ($\le 2^{2040}$ bits); all Pulsar-M tags above are
...
The hardness assumption underlying Pulsar-M is identical to the
...
Identical to the BDLOP18~\cite{ringtail2024} commitment-hiding
...
$\rho_B = \cSHAKE(\text{empty}, 256, \text{`Pulsar-M'},
\text{`PULSAR-M-EXPANDB-V1'}\,)$. The two derivations are
...
Pulsar-M's DKG produces a Shamir share of the FIPS~204 secret
...
Pulsar-M's signing protocol re-uses every distribution from FIPS~204
...
Pulsar-M must produce a $z$ vector with the same distribution as
...
All samplers used in Pulsar-M must be constant-time in their inputs.
...
The DKG produces a group public key $\mathit{pk}$ jointly distributed
as a fresh FIPS~204 ML-DSA public key, plus per-party Shamir shares
$(\mathbf{s}_i, \mathbf{u}_i)$ of the secret $\mathbf{s}_1$ and the
Pedersen blinding $\mathbf{u}$ respectively. Three rounds. Pedersen
commitments give verifiable secret sharing.
...
\[
H_i = \cSHAKE(\,\text{empty},\,256,\,\text{`Pulsar-M'},\,
\text{`PULSAR-M-DKG-COMMIT-V1'}\,||\,\mathit{encode}(\{\mathbf{C}_{i,k}\}_{k=0}^{t-1})\,).
\]
...
$\mathbf{s}_2$ is part of the private signing key; in Pulsar-M
...
as independent samples from $\chi_\eta$ (FIPS~204~\S5.1). Pulsar-M's
...
\State $\text{seed}_i \gets \KMAC(K_i^{\mathrm{prng}},\,\tau_1,\,256,\,\text{`PULSAR-M-SIGN-PRNG-V1'})$
...
\State Compute commit digest $D_i = \cSHAKE(\bar{\mathbf{w}}_i,\,256,\,\text{`Pulsar-M'},\,\text{`PULSAR-M-SIGN-R1-V1'} \| \tau_1)$
\State \textbf{For each} $j \in T \setminus \{i\}$ \textbf{do}
$\;\mathrm{MAC}_{i \to j} \gets \KMAC(K_{i,j},\,D_i \| \tau_1,\,256,\,\text{`PULSAR-M-SIGN-R1-MAC-V1'})$
...
\State Compute the aggregated commit hash $\bar D = \cSHAKE(\text{empty},\,256,\,\text{`Pulsar-M'},\,\text{`PULSAR-M-SIGN-R2-V1'} \,\|\, D_1 \| D_2 \| \cdots \| D_n)$
...
prefixing --- is exactly the FIPS~204 binding. Pulsar-M's $\Sign$
...
Pulsar-M signatures without modification.
...
reshare layer. Pulsar-M mitigates by deriving the quorum from the
...
\cSHAKE(\beta_e, 256\cdot|\mathcal{O}|, \text{`Pulsar-M'},
\text{`PULSAR-M-RESHARE-BEACON-V1'})\bigr\}.
\]
...
$(\text{`PULSAR-M-1'}, \text{kind\_byte}, \text{epoch\_u32})$.
...
FIPS~204~\S7.1, \texttt{bitlen}). Pulsar-M re-uses the FIPS~204
...
encoded \emph{exactly} per FIPS~204~\S7.2. \textbf{No Pulsar-M envelope.}
...
\texttt{PULSAR-M-RESHARE-TRANSCRIPT-V1} customisation tag.
...
reference (modulo Pulsar-M's switch from L2 norm to L-infty per
FIPS~204).
...
exactly the failure mode this rule prevents; the Pulsar-M reference
...
This is the hardest cryptographic engineering problem in Pulsar-M.
...
\paragraph{The Pulsar-M domain separation.}
Pulsar-M extends the Pulsar fix to handle three orthogonal
domains:
...
The Pulsar-M reference implementation enforces the cross-restart
...
(\texttt{bench/results/REPORT.md}, frozen at submission). Pulsar-M's
...
oracle is locked at \texttt{cmd/pulsar-m-oracle}.
...
\paragraph{Pulsar (R-LWE), Class~S~\cite{ringtail2024}.}
...
Different wire format. Pulsar-M shares Pulsar's protocol structure
...
commit-and-aggregate structure of Pulsar/Pulsar-M. Restricted to
$n$-out-of-$n$; Pulsar-M extends to $t$-out-of-$n$ via Shamir.
...
evaluation report; structural comparison: Olingo and Pulsar-M both
...
preview; THED was not public at Pulsar-M's draft cut-off.

\paragraph{HJKY97~\cite{hjky97}.}
The foundational proactive-resharing model. Pulsar-M's $\Reshare$
...
Pulsar-M-specific strengthening for mobile-adversary attacks.
...
$\mathbf{B} \in \Rq^{k \times \ell}$ & Pedersen blinding matrix, derived via \texttt{PULSAR-M-EXPANDB-V1} \\
$\mathbf{s}_1 \in \Rq^\ell$ & master secret, $\chi_\eta$-distributed \\
$\mathbf{u} \in \Rq^\ell$ & Pedersen blinding, $\chi_\eta$-distributed \\
$\mathbf{s}_2 \in \Rq^k$ & FIPS~204 noise, $= \mathbf{B}\mathbf{u}$ in Pulsar-M \\
...
of Pulsar-M; the threshold protocol is a port of the publicly-known
Ringtail~\cite{ringtail2024} construction with the beacon-quorum
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
\item \textbf{DKG commit-opening (CR-6).} The current Round-1
\texttt{cSHAKE256(c_i \| \mathit{blind}_i)} commit is broadcast but
$(c_i, \mathit{blind}_i)$ are never transmitted in any later round.
The commit binds nothing the protocol verifies. A malicious dealer
can broadcast an arbitrary commit and bias the joint public key.
Fix: include $(c_i, \mathit{blind}_i)$ in Round-2 reveal and verify
the digest opens, or drop \texttt{myCommit} entirely and document
the actual protocol (Shamir + sum).
\item \textbf{Sign-round session-key derivation (CR-7).}
\texttt{deriveMACKey} derives per-pair MAC keys from public inputs
$(\mathit{pk}_\mathrm{group}, \mathit{nodeID}_i, \mathit{nodeID}_j)$.
Any network observer computes the key and forges Round-1 messages,
breaking the commit-and-reveal binding and the identifiable-abort
property under no network partition. Fix: ephemeral session-key
exchange (Noise/X3DH/ML-KEM-768) bound to long-term ML-DSA identity.
\item \textbf{DKG envelope confidentiality (CR-8).} Round-1
envelopes carry per-recipient Shamir shares in plaintext on the
broadcast wire. A passive network observer recovers the master key
from one DKG ceremony. Fix: KEM-wrap envelopes (ML-KEM-768) per
recipient.
\item \textbf{Adaptive corruption proof (deferred).} The
\Cref{thm:euf-cma} unforgeability theorem is stated against the
...
\cite{ringtail2024} paper's adaptive variant uses a complexity
...
Pulsar-M variant without architectural changes? \emph{Unresolved.}
\end{openq}
...
binding to the Pulsar-M reshare MOB game requires a separate
hybrid argument bridging the consensus randomness assumption to
the Pulsar-M MOB game. \emph{Sketched in security-games.tex; full
proof round-2.}
\end{openq}
