% Pulsar-M security game definitions --- included by pulsar-m.tex
...
game (since the Pulsar-M $\Verify$ is unmodified ML-DSA
...
single-party unforgeability. Pulsar-M inherits ML-DSA's EUF-CMA
...
$\mathcal{O}_{\Sign}(\mu, T)$ runs Pulsar-M $\Sign$ honestly and
...
A Pulsar-M public key is $\mathit{pk}_{\mathrm{P}} = (\rho, \Power(\mathbf{A}\mathbf{s}_1 + \mathbf{B}\mathbf{u}, d))$ with $\mathbf{s}_1, \mathbf{u} \gets \chi_\eta$ (centred binomial) and $\mathbf{B}$ uniform.
...
equality} on a specific signature, the Pulsar-M $\Sign$
...
threshold lattice signatures is an open research question. Pulsar-M
...
parameter-pinned values (see \S\ref{sec:parameters}). Pulsar-M
