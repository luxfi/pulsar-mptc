...
as the single-party verified core under Pulsar-M's threshold layer. The
...
EasyCrypt) into the Pulsar-M submission tarball would:

1. Explode the submission tarball size.
2. Mix licenses (libjade is MIT, this repository is Apache-2.0) in a
   way that confuses provenance — the Pulsar-M submission should cite +
   link libjade, not redistribute it.
3. Couple the Pulsar-M submission's reproducibility to libjade's
...
(see the script) into `./libjade/`. The Pulsar-M Jasmin build then
...
| libjade artifact | Pulsar-M role |
|---|---|
...
Pulsar-M's threshold layer (`../threshold/`) wraps these as the
...
- libjade's ML-KEM (Kyber) tree — Pulsar-M is a signature scheme; KEM
  is out of scope.
- libjade's lower-parameter Dilithium variants (Dilithium2, Dilithium5).
  Pulsar-M's submitted parameter set is ML-DSA-65 only; ML-DSA-44 and
